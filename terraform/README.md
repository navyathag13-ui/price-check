...
The four gold summary tables have since been loaded into the SQL database (see `sql/azure_sql/README.md`). The storage account, Data Factory and Event Hubs are ready for the next stage (uploading the Delta history and scheduling loads). Event Hubs Basic bills about $0.03 per hour while it exists. Run `terraform destroy` (or destroy just the Event Hubs namespace) when you don't need it. The commands to run it yourself:
